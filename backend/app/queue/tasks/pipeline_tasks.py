...
async def _run_mil_analyze_recent(job: JobRun) -> dict:
    payload = job.payload_json or {}
    async with async_session() as db:
        return await mil_service.analyze_recent(
            db,
            hours=int(payload.get("hours") or 6),
            max_articles=int(payload.get("max_articles") or 300),
            include_competitors=bool(payload.get("include_competitors") or False),
            created_from_job_id=str(job.id),
        )
...
def run_mil_analyze_recent(self: Task, job_id: str) -> dict:
    try:
        result = run_async(
            _run_task_with_idempotency(
                job_id,
                task_name="mil_analyze_recent",
                runner=_run_mil_analyze_recent,
            )
        )
        run_async(_complete(job_id, result))
        return {"ok": True}
