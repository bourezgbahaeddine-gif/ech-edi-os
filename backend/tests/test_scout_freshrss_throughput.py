...
def test_freshrss_feed_url_adds_nb_limit(monkeypatch) -> None:
    monkeypatch.setattr(scout_module.settings, "freshrss_feed_url", "http://freshrss:80/i/?a=rss&state=all")
...
def test_freshrss_feed_url_keeps_operator_nb_override(monkeypatch) -> None:
    monkeypatch.setattr(scout_module.settings, "freshrss_feed_url", "http://freshrss:80/i/?a=rss&state=all&nb=120")
    monkeypatch.setattr(scout_module.settings, "freshrss_feed_limit", 1000)
    monkeypatch.setattr(scout_module.settings, "scout_max_new_per_run", 800)
...
def test_freshrss_feed_url_rewrites_legacy_public_path(monkeypatch) -> None:
