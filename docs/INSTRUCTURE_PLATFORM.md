...
- `journalist`: تحرير وتشغيل الأدوات دون اعتماد نهائي.
- `editor_chief`: اعتماد نهائي أو إرجاع وإدارة الطوابير التحريرية.
- `social_media`: تنفيذ واعتماد التدفقات الرقمية ضمن الصلاحيات المسموحة.
- `print_editor`: تحرير وتشغيل أسطح غرفة الأخبار والطباعة ضمن الصلاحيات المسموحة.
- `director`: صلاحيات كاملة + إعدادات النظام + المراقبة التشغيلية.
...
ملاحظة: `/health` يعرض `version` الحالية وحالة `database` و`redis` و`uptime_seconds`.
فحص قاعدة البيانات يتم عبر ping حي، وإذا تعطل DB أو Redis تبقى الاستجابة `200` لكن الحالة تصبح `degraded`.
